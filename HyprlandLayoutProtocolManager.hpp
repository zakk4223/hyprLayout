...
class CHyprlandLayoutProtocolManager {
	public:
		...
		void pushWindowDimensions(wl_client *client, wl_resource *resource, const char *window_id, uint32_t window_index, int32_t x, int32_t y, uint32_t width, uint32_t height, uint32_t serial);
		void commit(wl_client *client, wl_resource *resource, const char *layout_name, const char *config_data, uint32_t serial);
    void sendWindowInfo(wl_resource *resource, const char *window_id, int32_t x, int32_t y, uint32_t width, uint32_t height, uint32_t is_master, uint32_t is_active, uint32_t user_modified, const char *tags, uint32_t serial);
